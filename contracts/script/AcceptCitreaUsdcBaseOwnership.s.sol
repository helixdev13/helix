...
contract AcceptCitreaUsdcBaseOwnership is Script {
    ...
    function run() external {
        if (block.chainid != CITREA_MAINNET_CHAIN_ID) {
            revert("AcceptCitreaUsdcBaseOwnership must run on Citrea Mainnet");
        }
        ...
        if (
            ...
            revert("Final owner must be the pending owner on all base-vault contracts");
        }
