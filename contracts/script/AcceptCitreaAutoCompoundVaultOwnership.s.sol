...
contract AcceptCitreaAutoCompoundVaultOwnership is Script {
    ...
    function run() external {
        if (block.chainid != CITREA_MAINNET_CHAIN_ID) {
            revert("AcceptCitreaAutoCompoundVaultOwnership must run on Citrea Mainnet");
        }
        ...
        if (
            vaultFactory.pendingOwner() != config.finalOwner
                ...
                || rewardDistributor.pendingOwner() != config.finalOwner
        ) {
            revert("Final owner must be the pending owner on all auto-compound contracts");
        }
        ...
        rewardDistributor.acceptOwnership();
        ...
        console2.log("RewardDistributor owner:", rewardDistributor.owner());
    }
