...
contract TransferCitreaUsdcAllocatorShellOwnership is Script {
    ...
    function run() external {
        if (block.chainid != CITREA_MAINNET_CHAIN_ID) {
            revert("TransferCitreaUsdcAllocatorShellOwnership must run on Citrea Mainnet");
        }

        Config memory config = _loadConfig();
        if (config.finalOwner == address(0) || config.finalOwner == config.broadcaster) {
            revert("TransferCitreaUsdcAllocatorShellOwnership requires a distinct final owner");
        }
        ...
        if (
            vaultFactory.owner() != config.broadcaster || riskEngine.owner() != config.broadcaster
                || oracleRouter.owner() != config.broadcaster
                || vault.owner() != config.broadcaster || strategy.owner() != config.broadcaster
        ) {
            revert("Broadcaster must own all allocator shell contracts before transfer");
        }
